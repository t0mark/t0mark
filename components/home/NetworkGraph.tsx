...
// vis-network 옵션 및 데이터 (index.js 로직 이식)
const baseNodes = [
...
// 이미지 패딩 처리 (index.js padImageToRatio 이식)
function padImageToRatio(url: string, ratio: number): Promise<string> {
...
async function buildPaddedNodes(nodes: typeof baseNodes) {
  const imageUrls = nodes.filter((n) => n.image).map((n) => n.image)
...
export default function NetworkGraph() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const options = {
        ...
        physics: {
          ...
          repulsion: { centralGravity: 0, springConstant: 0.035, nodeDistance: 100, damping: 0.12 },
        },
        interaction: { hover: true, tooltipDelay: 200, dragNodes: true, dragView: false, zoomView: false },
      }

      const paddedNodes = await buildPaddedNodes(baseNodes)
      if (destroyed) return

      const nodes = new DataSet(paddedNodes)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const edges = new DataSet(edgesData as any[])
    ...
    const handleResize = () => {
      if (networkRef.current) {
        // @ts-expect-error vis-network type
  ...
  return <div ref={containerRef} id="network" className="w-full max-w-[1400px]" />
}
