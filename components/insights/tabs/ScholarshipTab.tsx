...
import type { ScholarshipItem, ScholarshipSource, ScholarshipsData } from '@/types/scholarships'

// ── 소스 메타데이터 ──────────────────────────────────────────────
const SOURCE_META: Record<ScholarshipSource, { label: string; bg: string; text: string; border: string }> = {
  saramin: {
    label: '사람인',
    bg: 'bg-red-50',
    text: 'text-red-700',
...
const ALL_SOURCES = Object.keys(SOURCE_META) as ScholarshipSource[]
...
// ── 소스 배지 ───────────────────────────────────────────────────
function SourceBadge({ source }: { source: ScholarshipSource }) {
  ...
  return (
    <div className="col-span-2 flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-xs text-text-light mb-4">
        아래 명령어로 산학장학생 공고를 수집하세요:
      </p>
      <div className="bg-bg-gray rounded-lg px-5 py-3 font-mono text-xs text-primary border border-border text-left space-y-1">
        <div>
          <span className="text-text-light"># 전체 수집 (GPT 필터 포함)</span>
        </div>
        <div>npm run fetch-scholarships</div>
        <div className="pt-1">
          <span className="text-text-light"># 빠른 테스트 (GPT 없음)</span>
        </div>
        <div>npm run fetch-scholarships:fast</div>
      </div>
    </div>
  )
}
...
export default function ScholarshipTab() {
  const [data, setData] = useState<ScholarshipsData>({ lastUpdated: null, items: [] })
  ...
  useEffect(() => {
    fetch('/api/insights/scholarships')
      .then((r) => r.json())
      .then((d: ScholarshipsData) => setData(d))
      .catch(console.error)
      .finally(() => setLoading(false))
  }, [])

  const countBySource = useMemo(() => {
    const map: Partial<Record<ScholarshipSource, number>> = {}
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h3 className="text-base font-bold text-primary flex items-center gap-2">
            <GraduationCap className="w-4 h-4" /> 로봇·AI R&D 산학 장학생
          </h3>
        </div>
        {data.lastUpdated && (
      ...
      {data.items.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {FILTER_OPTIONS.map((opt) => {
            const count =
              opt === '전체' ? data.items.length : (countBySource[opt as ScholarshipSource] ?? 0)
            if (opt !== '전체' && count === 0) return null
            const active = filter === opt
            const meta = opt !== '전체' ? SOURCE_META[opt as ScholarshipSource] : null
            return (
              <button
                key={opt}
